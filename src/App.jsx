...
function Bill({ billRegex, validation, onBillHandle, onBillType }) {
  ...
  //   if (!billRegex.test(e.target.value) && error || !billRegex.test(e.target.value) && !error) {
  //     e.target.classList.remove('success');
  //     e.target.classList.add('error');
  //     setError(true);
  //   } else if (billRegex.test(e.target.value) && error || billRegex.test(e.target.value) && !error) {
  ...
  return (
        <label className="bill_label">
          ...
          <div className="user_bill_input">
            ...
            <input type="text" placeholder="0" ref={billRef} onKeyPress={restrictBillInput} onChange={(e) => {
              validateBillInput(e);
              returnBillHandle(e);
            }} />
          </div>
...
function TipMenu({ onTipHandle, onTipChange, onTipClick, validation }) {
  ...
  return (
    <div className="tip_portion">
          ...
          <div className="tip_buttons">
            ...
            <input className="tip_button button_custom" type="text" placeholder="Custom" onKeyPress={handleKeyPress} onChange={(e) => {
...
function PeopleAmount({ numVal, onPeopleHandle, validation }) {
    ...
    <div className="amount_of_people_portion">
          <label>
            ...
            <div className="num_people_input">
              ...
              <input type="text" id="num_people" placeholder="0" ref={numPeopleRef} onKeyPress={handleCustomChange} onChange={(e) => {
  ...
  return (
    <div className="results_portion">
          <div className="amount_container">
            ...
            <p className="tip_amount amount_display">$0.00</p>
          </div>
          <div className="amount_container">
            <div className="amount_header">
            ...
            <p className="tip_amount total_display">$0.00</p>
          </div>
...
function App() {
  ...
  const validateBill = (e) => {
      if (e.target.value === '' || !regexes.bill_regex.test(e.target.value) && error.billError || !regexes.bill_regex.test(e.target.value) && !error.billError) {
        e.target.classList.remove('success');
        e.target.classList.add('error');
        setError((prev) => ({
          ...prev,
  ...
  const validatePeople = (e) => {
    if (e.target.value === '' || !regexes.num_people_regex.test(e.target.value) && error.numPeopleError || !regexes.num_people_regex.test(e.target.value) && !error.numPeopleError) {
      e.target.classList.remove('success');
      e.target.classList.add('error');
      setError((prev) => ({
        ...prev,
  ...
  // Validation function for the custom input that checks the input against the regex while also checking for error state
  const validateCustomInput = (e) => {
    if (e.target.value === '' || !regexes.custom_tip_regex.test(e.target.value) && error.CustomTipError || !regexes.custom_tip_regex.test(e.target.value) && !error.CustomTipError) {
      e.target.classList.remove('success');
      e.target.classList.add('error');
      setError((prev) => ({
        ...prev,
    ...
    } else if (regexes.custom_tip_regex.test(e.target.value) && error.customTipError || regexes.custom_tip_regex.test(e.target.value) && !error.customTipError) {
      e.target.classList.remove('error');
      e.target.classList.add('success');
      setError((prev) => ({
        ...prev,
  ...
  const returnTipAmountChange = (amount) => {
    ...
    setBillAmount((prev) => ({
      ...prev,
      tip: Number(amount.target.value),
    }))
  }
  ...
  const returnTipAmountClick = (amount) => {
    setBillAmount((prev) => ({
      ...prev,
      tip: Number(amount.target.textContent.replace(regexes.percent_regex, '')),
    }))
  ...
  return (
    <div className="container">
      ...
      <div className="calculator_container">
        ...
        <Bill validation={(e) => validateBill(e)} onBillType={(e) => handleBillChange(e)} onBillHandle={(e) => returnBillAmount(e)}  />
        {/* upper portion ends */}
        {/* tip portion begins */}
        <TipMenu validation={(e) => validateCustomInput(e)} onTipHandle={(e) => handleCustomChange(e)} onTipClick={(e) => returnTipAmountClick(e)} onTipChange={(e) => returnTipAmountChange(e)} />
        {/* tip portion ends */}
        {/* number of people portion begins */}
        <PeopleAmount validation={(e) => validatePeople(e)} onPeopleHandle={(e) => handleBillChange(e)}  numVal={(e) => returnNumOfPeople(e)} />
